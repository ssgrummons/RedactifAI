...
class TestDeidentificationTaskUnit:
    ...
    def test_task_handles_pipeline_failure(self, sample_tiff_bytes):
        """Test task handles pipeline failure and updates job accordingly."""
        job_id = str(uuid.uuid4())
        input_key = f"input/{job_id}.tiff"
        ...
        # Create mock task context with request.retries
        mock_self = Mock()
        mock_self.request.retries = 0
        
        with patch('src.tasks.DatabaseSessionManager', return_value=mock_db_manager), \
            patch('src.tasks.create_storage_backend', return_value=mock_phi_storage), \
            patch('src.tasks.asyncio.run') as mock_asyncio_run, \
            patch('src.tasks.celery_settings') as mock_settings:
            
            mock_settings.CELERY_TASK_MAX_RETRIES = 3
            
            # Download succeeds, but pipeline fails
            mock_asyncio_run.side_effect = [
                sample_tiff_bytes,  # download
                RuntimeError("Pipeline failed"),  # pipeline
            ]
            
            # Call __wrapped__ with mock self
            with pytest.raises(RuntimeError, match="Pipeline failed"):
                deidentify_document_task.__wrapped__(
                    mock_self,
                    job_id=job_id,
                    input_key=input_key,
                    masking_level="safe_harbor",
    ...
    def test_task_updates_retry_count_on_each_attempt(self, sample_tiff_bytes):
        """Test that retry count increments on each failed attempt."""
        job_id = str(uuid.uuid4())
        input_key = f"input/{job_id}.tiff"
        ...
        # Simulate second retry (first attempt + 2 retries)
        mock_task_self = Mock()
        mock_task_self.request.retries = 2
        
        with patch('src.tasks.DatabaseSessionManager', return_value=mock_db_manager), \
             patch('src.tasks.create_storage_backend'), \
             patch('src.tasks.asyncio.run') as mock_asyncio_run, \
             patch('src.tasks.celery_settings') as mock_settings:
            ...
            # Verify retry count reflects the attempt
            assert mock_job.retry_count == 3  # retries + 1
